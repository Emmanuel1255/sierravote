...
function createChart(description, labels, data) {
    try {
        var canvas = $('#' + description);
        if (!canvas.length) {
            console.error('Canvas not found for description:', description);
            return;
        }
        ...
        if (charts[description]) {
            charts[description].destroy();
        }
        ...
        charts[description] = new Chart(ctx).HorizontalBar(chartData, chartOptions);
    } catch (error) {
...
function updateVoteCounts() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // Update total votes count in separate request
            $.ajax({
                ...
                success: function(totals) {
                    if (totals.error) {
                        console.error('Error getting totals:', totals.message);
                        return;
                    }
                    $('#total_voters').html(totals.total_voters);
                    $('#voters_voted').html(totals.voters_voted);
                },
...
$(document).ready(function() {
    console.log('Initializing vote count updates...');
    updateVoteCounts();
